...
public static class Extensions
{
    ...
    {
        var typedArray = context.ObtainStyledAttributes(new int[] { Android.Resource.Attribute.SelectableItemBackground });
        var ripple = typedArray.GetDrawable(0) as RippleDrawable;

        ripple.SetColor(new(new int[][] { Array.Empty<int>() }, new int[] { Colors.Gray.WithAlpha(0.5f).ToInt() }));
